...
| `PauseMenu.ts` | `PauseMenu`, `PauseMenuSnapshot` | ESC pause overlay: Frutiger Aero / Mirror's Edge style (frosted backdrop, white panel, cyan accent). Pilot stats, Walker block, inventory list, Resume / Restart / Quit. `Game.buildPauseSnapshot()` feeds live character ratios when the menu opens. |
