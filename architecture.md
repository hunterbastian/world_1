...
| File | Exports | Purpose |
|------|---------|---------|
| `HUD.ts` | `HUD` | Fixed overlay: health/stamina/XP bar stack (top-left), compass rose (top-center), **layered SVG reticle** (gradient brackets, corner frame, rotating dashed range ring + glow pulse in pilot mode; minimal center stack in explore via `data-reticle-mode` + `style.css`), Walker health bar (piloting only), interaction prompt (bottom-center). |
