...
export function tierDims(tier: WalkerTier): TierDims {
  if (tier === 'scout') {
    return {
      bodyR: 1.05,
      bodySquashY: 0.62,
...
export function shadeColor(hex: number, mult: number) {
...
export function buildWalkerMechModel(tier: WalkerTier): { group: THREE.Group; limbs: WalkerLimbs } {
  ...
  const limbs: WalkerLimbs = { hull, head, weaponL, weaponR, legs, restHullY: 0 }

  const outlineThick = tier === 'scout' ? 0.04 : 0.06
  addOutlineShell(group, { thickness: outlineThick, color: 0x06060c, alpha: 0.65 })
