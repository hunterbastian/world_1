...
| Warm/cool color split | ToonRamp has a single warm shadow tint | Need separate warm highlight + cool shadow grading |
| Atmospheric perspective | HeightFog effect exists | Missing distance-based desaturation + blue shift |
...
| Camera feel | Decent FP rig | Needs boxing-style weight, lowered aim point, smoother transitions |
...
### From No Man's Sky (voxel direction)

Moving terrain to voxels would give: caves, overhangs, terrain editing, more organic shapes. But it's a **major rewrite** of the terrain system. Consider:

- **Hybrid approach**: keep heightmap for the base terrain (it's fast, already works). Add voxel-based cave/overhang features as overlays in specific POI zones.
- **Full voxel**: replace `Terrain.ts` entirely. Chunked 32³ density field, marching cubes in Web Workers, triplanar texturing. Enables the full NMS experience but costs weeks of engine work.
- **Visual only**: keep heightmap but push the *visual style* toward NMS — more saturated palettes, procedural texture variety, horizon-scale atmospheric rendering. This gets the NMS vibe without the architectural cost.

---

## Recommended Priority for Glasswake

1. **Camera feel** — Destiny's boxing-style weight, lowered aim point, smoother transitions
2. **Warm/cool color grading** — the single biggest visual identifier of the Destiny look
3. **Ambient particles** — pollen, dust, snow, fireflies. Huge atmosphere per effort
4. **Depth of field** — even subtle, adds cinematic quality
5. **Atmospheric perspective** — distance desaturation + blue shift for vast horizon feel
6. **Eye adaptation** — auto-exposure for interior/exterior transitions
7. **Tree shadows** — currently disabled, free depth in forests
8. **Voxel terrain exploration** — prototype chunked marching cubes for caves/overhangs (longer-term)
